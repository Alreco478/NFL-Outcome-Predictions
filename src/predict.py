...
def predict_winner(model, df, home_team, away_team):
    ...
    winning_probability = max(probabilities[0][1], probabilities[1][1]) / (probabilities[0][1] + probabilities[1][1]) * 100
    home_prob = probabilities[0][1]
    away_prob = probabilities[1][1]

    return predicted_winner, winning_probability, home_prob, away_prob
...
def predict_week(logreg_model, scaled_data, games):
    ...
    for home_team, away_team in games:
        ...
        predicted_winner, winning_probability, home_prob, away_prob = predict_winner(logreg_model, scaled_data, home_team, away_team)
        ...
        predictions.append({
            ...
            'Winning Probability (%)': round((winning_probability / 10) - 5, 2),
            'Home Win Probability (%)': round(home_prob, 2),
            'Away Win Probability (%)': round(away_prob, 2)
        })
