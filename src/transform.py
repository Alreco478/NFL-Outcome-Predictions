...
def remove_weeks(df, weeks_to_remove):
    """
    Filters the DataFrame to exclude entries from specified weeks.

    Args:
        df (pd.DataFrame): The DataFrame containing game data with a 'game_id' column.
        weeks_to_remove (list of int): A list of week numbers to remove from the DataFrame.

    Raises:
        ValueError: If df is not a pandas DataFrame or if the required column 'game_id' is not present.

    Returns:
        pd.DataFrame: A DataFrame with the specified weeks removed.
    """
    # Validate input types
    if not isinstance(df, pd.DataFrame):
        raise ValueError("Input must be a pandas DataFrame")
    ...
    if not isinstance(weeks_to_remove, list):
        raise ValueError("weeks_to_remove must be a list of integers")

    # Extract the week number from the 'game_id' column
    df['week'] = df['game_id'].str.extract(r'_(\d{2})_')[0].astype(int)

    # Print all unique weeks in the DataFrame
    unique_weeks = df['week'].unique()
    print(f"Unique weeks in the DataFrame: {sorted(unique_weeks)}")

    # Identify which weeks will be removed
    weeks_removed = [week for week in weeks_to_remove if week in unique_weeks]
    print(f"Weeks removed: {sorted(weeks_removed)}")

    # Filter out rows with weeks in the weeks_to_remove list
    df_filtered = df[~df['week'].isin(weeks_to_remove)]
...
def aggregate_df(df, group_by_list, column_func_df):
    ...
    # Ensure the dtype of each column in the aggregated DataFrame
    for column in aggregated.columns:
        if aggregated[column].dtype == 'object':
            aggregated[column] = pd.Series(aggregated[column], dtype='float64')
